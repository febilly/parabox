...
class Tileset:
    ...
    def init(self, base_canvas: int, temp_canvas: int, height: int=None, width: int=None):
    ...
    def draw_tile_xy(self, name: str, canvas: int, x: int, y: int):
        if not self.inited:
    ...
    def draw_tile_pos(self, name: str, canvas: int, x1, y1, x2, y2):
        self.draw_tile_size(name, canvas, x1, y1, x2 - x1 + 1, y2 - y1 + 1)

    def draw_tile_area(self, name: str, canvas: int, area: tuple[tuple[int, int], tuple[int, int]]):
